"""
PTSD Trigger Detection — Streamlit Dashboard (Redesigned)
==========================================================
...
def run_detection_cycle():
    ...
    if not ret:
        # Camera disconnected — try reopening
        st.session_state.webcam.release()
        del st.session_state["webcam"]
        return
    ...
    try:
        ...
        samples = int(AUDIO_SAMPLE_RATE * AUDIO_DURATION)
        audio = sd.rec(samples, samplerate=AUDIO_SAMPLE_RATE, channels=1, dtype="float32")
        sd.wait()
        audio_1d = audio.flatten()
        st.session_state.audio_volume = st.session_state.audio_clf.get_volume_level(audio_1d)
        audio_results = st.session_state.audio_clf.classify_audio(audio_1d)
...
with cam_col:
    st.markdown('<div class="sec-header">📷 Live Camera Feed</div>', unsafe_allow_html=True)
    with st.container():
        if st.session_state.frame_bytes:
            st.image(st.session_state.frame_bytes, channels="BGR", width="stretch")
            # Detected objects
            ...
            st.markdown("""
            <div style="background:#13161f;border:1px dashed #1e2130;border-radius:10px;
                        height:260px;display:flex;align-items:center;justify-content:center;
                        color:#5a6278;font-size:14px;flex-direction:column;gap:8px">
                <span style="font-size:28px">📷</span>
                Press <strong style="color:#e2e8f0">Start</strong> to begin monitoring
            </div>
            """, unsafe_allow_html=True)
